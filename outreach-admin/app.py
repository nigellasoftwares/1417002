...
# Staff Server Req.
@app.route("/staff-server-req", methods=['POST'])
def staffServerReq():
    if(request.method == 'POST'):
        try:
            ...
            cur = conn.cursor()
            adminServerIP = ip_address
            
            if adminServerIP == serverIP:
                response = {
                    'status' : 'success' 
                }
...
#  staff-dermalog-ip-address
@app.route("/staff-dermalog-ip-address", methods=['POST'])
def staff_staff_dermalog_ip_address():
    if(request.method == 'POST'):
        try:
            conn = get_db_connection()
            cur = conn.cursor()
            ...
            dermalog_ip = cur.fetchall()
            dermalog_ip_len = len(dermalog_ip)

            if dermalog_ip_len == 1:
                response = {
                    'status' : 'success',
                    'dermalog_ip': dermalog_ip[0][0]
                }
                return jsonify(response)
            else:
                response = {
...
@app.route("/staff-verification", methods=['POST'])
def staffVerification():
    if(request.method == 'POST'):
        #data = request.get_json()
        #print(data)
        username = request.form.get('username')
        password = request.form.get('password')
        staff_server_ip = request.form.get('staff_server_ip')
        staff_server_port = request.form.get('staff_server_port')
        ...
        if username and password:
            try:
                # Staff Auth
                staff = cur.execute("SELECT email FROM staff WHERE email=? AND password=?", (username, password))
                staff = staff.fetchall()
                staffLen = len(staff)
                if staffLen == 1:
                    # update staff
                    cur.execute("SELECT staff_username FROM staff_connection_status WHERE staff_username=?", (username,))
                    staff_connection = cur.fetchall()
                    staff_connection_len = len(staff_connection)

                    if staff_connection_len == 1:
                        cur.execute("UPDATE staff_connection_status SET staff_server_ip=?, staff_server_port=? WHERE staff_username=?", (staff_server_ip, staff_server_port, username))
                        conn.commit()
                        conn.close()
                    elif staff_connection_len == 0:
                        cur.execute("INSERT INTO staff_connection_status(staff_username, staff_server_ip, staff_server_port) VALUES(?, ?, ?)", (username, staff_server_ip, staff_server_port))
                        conn.commit()
                        conn.close()
...
# Staff 
@app.route("/staff-my-registration-list-request", methods=['POST'])
def staffMyRegistrationListRequest():
    if(request.method == 'POST'):
        ...
        if search_data:
            try:
                cur.execute("SELECT id, form_worker_reg_no, worker_detail_name_of_worker FROM half_form WHERE form_worker_reg_no=?", (search_data,))
...
# Staff upload worker docs
@app.route("/staff-upload-worker-docs", methods=['POST'])
def staff_uploadWorkerDocs():
    if(request.method == 'POST'):
        res= {}
        workerRegNum = request.form.get('workerRegNum')
        docNum = request.form.get('docNum')
        avatar = request.form.get('avatar')
        
        print('WDocs:: ', workerRegNum, docNum, avatar)
...
# Staff fetch worker docs
@app.route("/staff-fetch-worker-docs", methods=['POST'])
def staff_fetchWorkerDocs():
    if(request.method == 'POST'):
        res= {}
        workerRegNum = request.form.get('workerRegNum')
        docNum = request.form.get('docNum')
        
        print('F-WDocs:: ', workerRegNum, docNum)

        # Database
        conn = get_db_connection()
        cur = conn.cursor()

        if workerRegNum and docNum:
            try:
                # check query
...
# Staff upload FM docs
@app.route("/staff-upload-fMember-docs", methods=['POST'])
def staff_upload_fMemberDocs():
    if(request.method == 'POST'):
        res= {}
        workerRegNum = request.form.get('workerRegNum')
        docNum = request.form.get('docNum')
...
# Staff fetch FM docs
@app.route("/staff-fetch-fMember-docs", methods=['POST'])
def staff_fetch_fMemberDocs():
    if(request.method == 'POST'):
        res= {}
        workerRegNum = request.form.get('workerRegNum')
        docNum = request.form.get('docNum')
...
# Staff worker data
@app.route("/staff-view-registered-worker-req", methods=['POST'])
def staffViewRegisteredWorkerReq():
    if(request.method == 'POST'):
        ...
        if workerId:
            try:
                ...
                if workerLen >= 1:
                    response = {
                        'status': 'success',
                        'workerBiodata': workerBiodata,
                        'workerRegTime': workerRegTime,
                        'workerList': workerD,
...
# staff view family member
@app.route("/staff-view-family-member-req", methods=['POST'])
def staffViewFamilyMemberReq():
    if(request.method == 'POST'):
        ...
        if fmId:
            try:
                ...
                if workerRegN:
                    ...
                    worker_reg_no = workerRegN[0][1]

                # fetch FM Docs
                cur.execute("SELECT * FROM fm_document WHERE fm_reg_no=?", (fm_reg_no,))
                fmDocs = cur.fetchall()
                
                # FM BioData
                cur.execute("SELECT * FROM fm_biodata WHERE fm_reg_no=?", (fm_reg_no,))
...
# staff edit family member
@app.route("/staff-edit-family-member-req", methods=['POST'])
def staff_editFamilyMemberReq():
    if(request.method == 'POST'):
        fmId = request.form.get('fmID')
...
# Staff edit-registered worker
@app.route("/staff-edit-registered-worker-req", methods=['POST'])
def staffeditRegisteredWorkerReq():
    if(request.method == 'POST'):
        workerId = request.form.get('workerID')
        print(workerId)
...
# Staff 
@app.route("/staff-company-list-req", methods=['GET'])
def staffCompanyListReq():
    if(request.method == 'GET'):
        conn = get_db_connection()
        cur = conn.cursor()

        try:
            cur.execute("SELECT worker_registration_prefix FROM company_list")
            companyLs = cur.fetchall()
            com_len = len(companyLs)
           
            if com_len >= 1:
                response = {
                    'status': 'success',
                    'company_list': companyLs
                }
                return jsonify(response)
            else:
                response = {
...
# Staff confirm-company-profile
@app.route("/staff-confirm-company-profile", methods=['POST'])
def staff_confirmCompanyProfile():
    if(request.method == 'POST'):
        compName = request.form.get('company_name')
        
        conn = get_db_connection()
        cur = conn.cursor()

        try:
            cur.execute("SELECT username FROM company_list where worker_registration_prefix=?", (compName,))
            user = cur.fetchall()
            user_len = len(user)
            
            if user_len >= 1:
                username = user[0][0]
                cur.execute("SELECT aps_email FROM profile where aps_email=?", (username,))
...
# Staff get-company-profile
@app.route("/staff-get-company-profile", methods=['POST'])
def staff_getCompanyProfile():
    if(request.method == 'POST'):
        username = request.form.get('username')
        ...
        try:
            cur.execute("SELECT branch_address1, branch_address2, branch_address3, branch_employment_location_name, branch_state, branch_city FROM profile WHERE aps_email=?", (username,))
            profile = cur.fetchall()
            profile_len = len(profile)
...
def staffProfile():
    if(request.method == 'POST'):
        ...
        try:
            cur.execute("SELECT * FROM staff WHERE email=?", (username,))
            profile = cur.fetchall()
            profile_len = len(profile)
...
# Staff Worker list
@app.route("/staff-worker-list", methods=['POST'])
def staffWorkerList():
    if(request.method == 'POST'):
        username = request.form.get('username')
        ...
        try:
            cur.execute("SELECT form_created_date, form_worker_reg_no, worker_detail_name_of_worker FROM half_form WHERE form_created_by=?", (username,))
            worker = cur.fetchall()
...
@app.route("/staff-employment-detail", methods=['GET'])
def staff_employmentDetail():
    if(request.method == 'GET'):
        ...
        try:
            cur.execute("SELECT employment_detail FROM employment_detail")
            profile = cur.fetchall()
...
# Staff Citizenship
@app.route("/staff-citizenship", methods=['GET'])
def staff_citizenship():
    if(request.method == 'GET'):
        ...
        try:
            ...
            citizenship_len = len(citizenship)

            if citizenship_len >= 1:
                response = {
                    'status': 'success',
...
# Staff marital
@app.route("/staff-marital", methods=['GET'])
def staff_marital():
    if(request.method == 'GET'):
        ...
        try:
            ...
            marital = cur.fetchall()
            
            marital_len = len(marital)
...
# Staff poe
@app.route("/staff-poe", methods=['GET'])
def staff_poe():
    if(request.method == 'GET'):
...
# Staff gender
@app.route("/staff-gender", methods=['GET'])
def staff_gender():
    if(request.method == 'GET'):
...
# Staff religion
@app.route("/staff-religion", methods=['GET'])
def staff_religion():
    if(request.method == 'GET'):
...
# Staff race
@app.route("/staff-race", methods=['GET'])
def staff_race():
    if(request.method == 'GET'):
...
# Staff relationship
@app.route("/staff-relationship", methods=['GET'])
...
# Staff job_sector
@app.route("/staff-job-sector", methods=['GET'])
def staff_job_sector():
    if(request.method == 'GET'):
...
# Staff job_status_sponsor
@app.route("/staff-job-status-sponsor", methods=['GET'])
def staff_job_status_sponsor():
    if(request.method == 'GET'):
        ...
        try:
            ...
            job_status_sponsor_len = len(job_status_sponsor)

            if job_status_sponsor_len >= 1:
                response = {
                    'status': 'success',
                    'job_status_sponsor': job_status_sponsor
                }
                return jsonify(response)
            else:
                response = {
...
# Staff city
@app.route("/staff-city", methods=['GET'])
def staff_city():
    if(request.method == 'GET'):
...
# Staff state
@app.route("/staff-state", methods=['GET'])
def staff_state():
    if(request.method == 'GET'):
...
# Staff issuingCountry
@app.route("/staff-issuingCountry", methods=['GET'])
def staff_issuingCountry():
    if(request.method == 'GET'):
        ...
        try:
            ...
            issuingCountry_len = len(issuingCountry)

            if issuingCountry_len >= 1:
                response = {
                    'status': 'success',
...
# Staff docStatus
@app.route("/staff-docStatus", methods=['GET'])
def staff_docStatus():
    if(request.method == 'GET'):
...
# Staff curDocStatus
@app.route("/staff-curDocStatus", methods=['GET'])
def staff_curDocStatus():
    if(request.method == 'GET'):
        ...
        try:
            ...
            curDocStatus_len = len(curDocStatus)

            if curDocStatus_len >= 1:
                response = {
                    'status': 'success',
                    'curDocStatus': curDocStatus
                }
                return jsonify(response)
            else:
                response = {
...
# Staff typeOfDoc
@app.route("/staff-typeOfDoc", methods=['GET'])
def staff_typeOfDoc():
    if(request.method == 'GET'):
...
@app.route("/staff-employment-status", methods=['GET'])
def staff_employment_status():
    if(request.method == 'GET'):
        ...
        try:
            ...
            employment_status = cur.fetchall()
            
            employment_status_len = len(employment_status)

            if employment_status_len >= 1:
                response = {
                    'status': 'success',
...
# Insert Worker
@app.route("/staff-insert-worker", methods=['POST'])
def staff_insertWorker():
    conn = get_db_connection()
    cur = conn.cursor()
    ...
    if request.method == 'POST':
        try:
            dataO = request.get_json()

            active_user_session = dataO.get('username')
...
# Staff Update Registered Worker
@app.route("/staff-update-registered-worker", methods=['POST'])
def staff_updateRegisteredWorker():
    if request.method == 'POST':
        try:
...
# Staff Update Registered Worker Docs
@app.route("/staff-update-worker-docs", methods=['POST'])
def staff_updateWorkerDocs():
    if request.method == 'POST':
        try:
            conn = get_db_connection()
            cur = conn.cursor()
...
# Staff Update worker Biodata
@app.route("/staff-update-worker-biodata", methods=['POST'])
def staff_updateWorkerBiodata():
    if request.method == 'POST':
        try:
            conn = get_db_connection()
            cur = conn.cursor()

            active_user_session = request.form.get('active_user_session')
...
# Staff Update Family Member
@app.route("/staff-update-family-member", methods=['POST'])
def staff_updateFamilyMember():
    if request.method == 'POST':
        try:
            active_user_session = session.get('user_session')
            conn = get_db_connection()
...
# Staff Update Family Member Docs
@app.route("/staff-update-family-member-docs", methods=['POST'])
def staff_updateFamilyMemberDocs():
    if request.method == 'POST':
        try:
            conn = get_db_connection()
            cur = conn.cursor()
            ...
            # Update FM details
            cur.execute("UPDATE fm_document SET type_of_douments=?, document_id=?, place_of_issue=?, document_issued_date=?, document_expiry_date=?, issuing_country=?, document_status=?, status_of_current_document=?  WHERE id=?", (type_of_document, document_id, place_of_issue, issue_date, expiry_date, issuing_country, document_status, document_current_status, docs_id))
            conn.commit()
            conn.close()
...
# Staff Update FM Biodata
@app.route("/staff-update-fm-biodata", methods=['POST'])
def staff_updateFMBiodata():
    if request.method == 'POST':
        try:
            conn = get_db_connection()
            cur = conn.cursor()
...
def adminDash():
    if 'admin_session' in session:
        ...
        return render_template('admin-dash.html',ip_address=ip_address, plantation_name=plantation_name, total_legal_worker=total_legal_worker, total_undocumented_worker=total_undocumented_worker, total_legal_fm=total_legal_fm, total_undocumented_fm=total_undocumented_fm, total_pati_imm=total_pati_imm, totalData=totalData)
    
    else:
        return redirect('/')


# admin Staff Connection Status 
@app.route('/admin-staff-connection-status')
def admin_staffConnectionStatus():
    if 'admin_session' in session:
        try:
            # Date & Time
            ...
            # Check Outreach
            cur.execute("SELECT * FROM staff_connection_status")
            staff_status = cur.fetchall()
            sLen = len(staff_status)
            ...
            if sLen >= 1:
                for staff in staff_status:
                    ...
                    try:
                        ...
                        if response.status_code == 200:
                            nStaff = (username, server_ip, 'connected')
                            new_staff_ls.append(nStaff)  
                    except requests.exceptions.RequestException:
                        nStaff = (username, server_ip, 'disconnected')
                        new_staff_ls.append(nStaff)

                print(new_staff_ls)
                return render_template('admin-staff-connection-status.html', staff_ls=new_staff_ls)
            else:
                return render_template('admin-staff-connection-status.html')
...
# admin active Staff
@app.route('/admin-active-staff', methods=['GET'])
def admin_activeStaff():
    if 'admin_session' in session:
        try:
            # DB
            conn = get_db_connection()
            ...
            cur.execute("SELECT staff_server_ip, staff_server_port FROM staff_connection_status")
            staff_status = cur.fetchall()
            sLen = len(staff_status)
...
@app.route('/admin-document-status')
def adminDocumentStatus():
    if 'admin_session' in session:
        try:
            conn = get_db_connection()
            cur = conn.cursor()
            cur.execute("SELECT * FROM detailed_dd_doc_status")
            dataList = cur.fetchall()
            return render_template('admin-document-status.html', dataList=dataList)
        except Exception as e:
            return redirect('/')
    else:
        return redirect('/')
    
# admin Create Document-status
@app.route('/admin-create-document-status', methods=['POST'])
def adminCreateDocumentStatus():
    if 'admin_session' in session:
        try:
            doc_status = request.form.get('document_status')
            # DB
            conn = get_db_connection()
            cur = conn.cursor()

            cur.execute('SELECT doc_status FROM detailed_dd_doc_status WHERE doc_status=?', (doc_status,))
            docStat = cur.fetchall()
            docStat_len = len(docStat)
            if docStat_len == 0:
                cur.execute('INSERT INTO detailed_dd_doc_status(doc_status) values(?)', (doc_status,))
                conn.commit()
                conn.close()
                session['admin_create_document_status'] = 'success'
                return redirect('/admin-document-status')
            else:
                session['admin_create_document_status'] = 'error'
                return redirect('/admin-document-status')      
        except Exception as e:
            session['admin_create_document_status'] = 'error'
            return redirect('/admin-document-status')       
...
# Admin Del Document-status
@app.route('/admin-del-document-status', methods=['GET'])
def adminDel_documentStatus():
    if 'admin_session' in session:
        try:
            ...
            cur.execute('DELETE FROM detailed_dd_doc_status WHERE id=?', (delID,))
            conn.commit()
            conn.close()
            session['admin_del_document_status'] = 'success'
            return redirect('/admin-document-status')
        except Exception as e:
            session['admin_del_document_status'] = 'error'
            return redirect('/admin-document-status')       
    else:
        return redirect('/') 
    

# Admin Employment Details
@app.route('/admin-employment-detail')
def admin_employmentDetail():
    if 'admin_session' in session:
        try:
            conn = get_db_connection()
            cur = conn.cursor()
            cur.execute("SELECT * FROM employment_detail")
            dataList = cur.fetchall()
            return render_template('admin-employment-detail.html', dataList=dataList)
        except Exception as e:
            return redirect('/')
    else:
        return redirect('/')
    
# admin Create employment_detail
@app.route('/admin-create-employment-detail', methods=['POST'])
def adminCreate_employment_detail():
    if 'admin_session' in session:
        try:
            employment_detail = request.form.get('employment_detail')
            # DB
            conn = get_db_connection()
            cur = conn.cursor()

            cur.execute('SELECT employment_detail FROM employment_detail WHERE employment_detail=?', (employment_detail,))
            docStat = cur.fetchall()
            docStat_len = len(docStat)
            if docStat_len == 0:
                cur.execute('INSERT INTO employment_detail(employment_detail) values(?)', (employment_detail,))
                conn.commit()
                conn.close()
                session['admin_create_employment_detail'] = 'success'
                return redirect('/admin-employment-detail')
            else:
                session['admin_create_employment_detail'] = 'error'
                return redirect('/admin-employment-detail')      
        except Exception as e:
            session['admin_create_employment_detail'] = 'error'
            return redirect('/admin-employment-detail')       
...
# Admin Del employment_detail
@app.route('/admin-del-employment-detail', methods=['GET'])
def adminDel_employment_detail():
    if 'admin_session' in session:
        try:
            delID = request.args.get('del')
            # DB
            ...
            cur.execute('DELETE FROM employment_detail WHERE id=?', (delID,))
            conn.commit()
            conn.close()
            session['admin_del_employment_detail'] = 'success'
            return redirect('/admin-employment-detail')
        except Exception as e:
            session['admin_del_employment_detail'] = 'error'
            return redirect('/admin-employment-detail')       
    else:
        return redirect('/') 
    ...
    if 'admin_session' in session:
        if request.method == 'POST':
            try:
                ...
                # Check Staff
                staff = cur.execute('SELECT email FROM staff WHERE email=?', (user_email,))
                staff = cur.fetchall()
                staffLen = len(staff)
                if staffLen == 1:
                    session['admin_create_staff'] = 'user_exist'
                    return render_template('admin-create-staff.html')
                else:
                    cur.execute('INSERT INTO staff(name, email, password, created_at, created_time) values(?, ?, ?, ?, ?)', (user_name, user_email, user_password, created_at, created_time))
                    conn.commit()
                    conn.close()

                    session['admin_create_staff'] = 'success'
                    return render_template('admin-create-staff.html')
            
            except Exception as e:
                session['admin_create_staff'] = 'error'
                return redirect('/admin-create-staff')
            
        else:
            session['admin_create_staff'] = 'method_error'
            return redirect('/admin-create-staff')        
    else:
        return redirect('/')
...
def adminViewStaff():
    if 'admin_session' in session:
        try:
            ...
            staff = cur.execute('SELECT id, name, email, password FROM staff')
            staff = cur.fetchall()
            ...
            conn.commit()
            conn.close()
            return render_template('admin-view-staff.html', totalUsers=totalstaff, userList=staffList)
        except Exception as e:
            return redirect('/admin-dash')       
    else:
        return redirect('/') 
...
# Admin Edit Staff
@app.route('/admin-edit-staff', methods=['GET'])
def adminEditStaff():
    if 'admin_session' in session:
        try:
            # DB
            ...
            users = cur.execute('SELECT id, name, email, password FROM staff WHERE id=?', (userID,))
            users = cur.fetchall()
            userList = users
            
            conn.commit()
            conn.close()
            return render_template('admin-edit-staff.html', userList=userList)
        except Exception as e:
            return redirect('/admin-dash')       
    else:
        return redirect('/')
...
def adminUpdateStaff():
    if 'admin_session' in session:
        try:
            ...
            cur.execute('UPDATE staff SET name=?, password=? WHERE id=?', (user_name, user_password, userID))
            conn.commit()
            conn.close()
            
            session['admin_update_staff'] = 'success'
            return redirect('/admin-view-staff')
        except Exception as e:
            session['admin_update_staff'] = 'error'
            return redirect('/admin-view-staff')       
    else:
        return redirect('/') 
...
# Admin Del Staff
@app.route('/admin-del-staff', methods=['GET'])
def adminDelStaff():
    if 'admin_session' in session:
        try:
            userID = request.args.get('usI')
            ...
            cur.execute('DELETE FROM staff WHERE id=?', (userID,))
            conn.commit()
            conn.close()
            session['admin_del_staff'] = 'success'
            return redirect('/admin-view-staff')
        except Exception as e:
            session['admin_del_staff'] = 'error'
            return redirect('/admin-view-staff')       
    else:
        return redirect('/') 
        ...
        if request.method == 'POST':
            ...
            if response.status_code == 200:
                ...
                if req_data['status'] == 'success':
                    ...
                    # Company list
                    companyList = data[20]['company_list']
                    for comp in companyList:
                        compD = tuple(comp)
                        
                        cl_check = comp[1]
                        cur.execute("SELECT username from company_list WHERE username=?", (cl_check,))
                        cl_check_len = cur.fetchall()
                        cl_check_len = len(cl_check_len)
                        if cl_check_len == 0:
                            cur.execute("INSERT INTO company_list(worker_registration_prefix, username) VALUES (?, ?)", compD)


                    # Profile
                    profileList = data[21]['profile']
                    for prof in profileList:
                        prof_d = tuple(prof)
                        
                        prof_check = prof[11]
                        cur.execute("SELECT aps_email from profile WHERE aps_email=?", (prof_check,))
                        prof_check_len = cur.fetchall()
                        prof_check_len = len(prof_check_len)
                        if prof_check_len == 0:
                    ...
                    session['admin_pull_data'] = 'error'
                    redirect('/admin-online-server-sync')
            else:
                session['admin_pull_data'] = 'error'
                return render_template('/admin-online-server-sync')
        else:
            session['admin_pull_data'] = 'error'
            return redirect('/admin-online-server-sync')
    else:
...
def adminViewRegisteredWorker():
    if 'admin_session' in session:
        try:
            ...
            return render_template('admin-view-registered-worker.html',workerBiodata=workerBiodata,  workerRegTime=workerRegTime, workerList=workerD, workerDocs=workerDocs, workerFM=workerFM)
        except Exception as e:
...
def adminViewFamilyMember():
    if 'admin_session' in session:
        try:
            ...
            cur.execute("SELECT * FROM fm_document where fm_reg_no=?", (fmRegNo,))
            fmDocs = cur.fetchall()
            ...
            return render_template('admin-view-family-member.html', fmBiodata=fmBiodata, wori=workerId, worReN=workerRegNo, fmN=fmN, fmData=fmData, fmDocs=fmDocs)
        except Exception as e:
            return redirect('/admin-registration-list')
    else:
        return redirect('/')
...
def get_dermalog_ip_status(serverIP):
    try:
        '''
        url = f"{serverIP}/SIFWAPI"
        print('print Server:: ', url)
        response = requests.get(url, timeout=2)
        if response.status_code == 200:
            print('print Server:: ', url, 'stat code -> ', response.status_code)
            return 'success'
        else:
            print('print Server:: ', url, 'stat code -> ', response.status_code)
            return 'error'
        ...
        SIFW_API_BASE_URL = f"{serverIP}/SIFWAPI"
        USERNAME = "SIFWuser1"
...
# fetch-dermalog-server-status
@app.route("/fetch-dermalog-server-status", methods=['POST'])
def admin_fetch_dermalog_server_status():
    try:
        data = request.get_json()
        serverIP = data.get('server_ip')
        
        if serverIP:
            status = get_dermalog_ip_status(serverIP)
            res = {
                'status': status
            }
            return jsonify(res)
...
def check_connectivity():
    dermalog_ip = request.args.get('ip')

    # Use the `socket` module to check if the IP is reachable
    try:
        socket.create_connection((dermalog_ip, 80), timeout=10)
        status = "connected"
        print ("connncted")
...
'''
# New route to check connectivity with the Dermalog IP
@app.route('/check-connectivity', methods=['GET'])
def check_connectivity():
    dermalog_ip = request.args.get('ip')

    try:
        response = requests.get(dermalog_ip, timeout=10)
        if response.status_code == 200:
            status = "connected"
            return True
        else:
...
def start_webview():
    ...
    webview.create_window("FW Registration System Outreach Admin", f"http://{ip_address}:6787/", js_api=True)
    webview.start()
