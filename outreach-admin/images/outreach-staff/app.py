...
def connectServer():
    if request.method == 'POST':
        try:
            ...
            '''
            response = connectionStaffServer(serverIP)
            ...
            if response['status'] == 'success':
                ...
                return redirect('/')
            '''
            # Pass the serverIP as JSON response
            return jsonify({"serverIP": serverIP})
        except Exception as e:
            session['auth_error'] = 'not_connected'
            return redirect('/')
    else:
...
def staff_verification_request(getIP, port, username, password):
    ...
    try:
        response = requests.post(url, data=allData)
...
@app.route('/my-reg-list-request', methods=['POST'])
def staff_reg_list_request():
    if 'user_session' in session:
        searchData = request.form.get('request_data')
        print(searchData)
        # Database
        conn = get_db_connection()
        cur = conn.cursor()
...
def profile():
    if 'user_session' in session:
        ...
        cur.execute("SELECT name, email, password FROM users where email=?", (active_user_session,))
        users = cur.fetchall()
        usersLen = len(users)
        if usersLen == 1:
            name = users[0][0]
        ...
        return render_template('profile.html', name=name, password=password, email=email, your_email=active_user_session, licenseCategoryList=licenseCategory, cityList=city, stateList=state, sectorList=sector, designationList=designation)
    else:
        return redirect('/')
