...
def get_local_ip():
    try:
        ...
        #  local IP address from the socket
    ...
    if 'username' in session:
        if request.method == 'POST':
            try:
                ...
                # save Worker data
                conn.execute('INSERT INTO half_form(form_worker_reg_no, no_family_mem, worker_detail_worker_legal_status,worker_detail_name_of_worker, worker_detail_family_name) VALUES("'+worker_registration_no+'", "'+no_of_family_member+'", "'+worker_legal_status+'", "'+name_of_worker+'", "'+family_name+'")')
                conn.commit()
                conn.close()  
...
def profileFormData():
    if 'username' in session:
        ...
        checkPro = conn.execute("SELECT * FROM profile")
        if(checkPro == 1):
